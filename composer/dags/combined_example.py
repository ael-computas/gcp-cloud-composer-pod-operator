...
with models.DAG(
    ...
    source_1_task = BashOperator(
        bash_command="echo \"Pretending to be source 1\"; sleep 30",
        task_id="source_1",
    )
    source_2_task = BashOperator(
        bash_command="echo \"Pretending to be source 2\"; sleep 30",
        task_id="source_2",
    )
    processing_tasks = []
    for i in range(1, 11):
        processing_tasks.append(kubernetes_pod_operator.KubernetesPodOperator(
            task_id='pod-workshop-simple_{}'.format(i),
            name='pod-workshop-simple-{}'.format(i),
            cmds=["sh", "-c", 'echo \'Sleeping..\'; sleep 30; echo \'Done!\''],
            namespace='default',
